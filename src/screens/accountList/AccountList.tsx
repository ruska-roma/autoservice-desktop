...
export function AccountList() {
  ...
  const { stateRef, save } = useScreenPersistence(SCREENS.AccountList);

  const tableRef = useRef<HTMLDivElement | null>(null);

  const [count, setCount] = useState(0);
  const [accounts, setAccounts] = useState<ExtAccountType[]>([]);
  ...
  const [selectedSearchField, setSelectedSearchField] = useState(
    stateRef.current.selectedField ?? ACCOUNT_SEARCH_FIELDS[0].key,
  );
  ...
  useEffect(() => {
    ...
    Promise.all([
      getAccountsList({ limit: DEFAULT_TABLE_PAGE_SIZE, offset }),
      getAccountsCount(),
    ]).then(([data, total]) => {
      setAccounts(data ?? []);
      setCount(total ?? 0);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      searchAccountsCount({ field: selectedSearchField, query: debouncedQuery }),
    ]).then(([data, total]) => {
      if (!isCurrent) {
        return;
      }
  ...
  const handleTableItemClick = (id: number) => {
    save();
    navigate(`/account/${id}`, { state: { fromPage: SCREENS.AccountList } });
  };

  const searchFields = useMemo(
    () =>
      createListCollection({
        items: ACCOUNT_SEARCH_FIELDS.map((field) => ({
          label: field.label,
          value: field.key,
        })),
      }),
  ...
  const searchFieldPlaceholder = useMemo(() => {
    const selectedField = ACCOUNT_SEARCH_FIELDS.find((f) => f.key === selectedSearchField);
  ...
  const isDataEmpty = !isSearchMode && !accounts.length && count === 0;
